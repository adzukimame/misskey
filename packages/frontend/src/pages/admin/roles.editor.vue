...
<div class="_gaps">
	...
	<MkSelect v-model="rolePermission" :readonly="readonly">
		...
		<template #caption><div v-text="i18n.ts._role.descriptionOfPermission"></div></template>
	...
	<MkSelect v-model="role.target" :readonly="readonly">
		...
		<template #caption><div v-text="i18n.ts._role.descriptionOfAssignTarget"></div></template>
	...
	<FormSlot>
		...
		<div class="_gaps_s">
			...
			<MkFolder v-if="matchQuery([i18n.ts._role._options.canMakePureRenoteOfRemoteNotes, 'canMakePureRenoteOfRemoteNotes'])">
				...
				<template #suffix>
					<span v-if="role.policies.canMakePureRenoteOfRemoteNotes.useDefault" :class="$style.useDefaultLabel">{{ i18n.ts._role.useBaseValue }}</span>
					<span v-else>{{ role.policies.canMakePureRenoteOfRemoteNotes.value ? i18n.ts.yes : i18n.ts.no }}</span>
					<span :class="$style.priorityIndicator"><i :class="getPriorityIcon(role.policies.canMakePureRenoteOfRemoteNotes)"></i></span>
				</template>
				<div class="_gaps">
					<MkSwitch v-model="role.policies.canMakePureRenoteOfRemoteNotes.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts._role.useBaseValue }}</template>
					</MkSwitch>
					<MkSwitch v-model="role.policies.canMakePureRenoteOfRemoteNotes.value" :disabled="role.policies.canMakePureRenoteOfRemoteNotes.useDefault" :readonly="readonly">
						<template #label>{{ i18n.ts.enable }}</template>
					</MkSwitch>
					<MkRange v-model="role.policies.canMakePureRenoteOfRemoteNotes.priority" :min="0" :max="2" :step="1" easing :textConverter="(v) => v === 0 ? i18n.ts._role._priority.low : v === 1 ? i18n.ts._role._priority.middle : v === 2 ? i18n.ts._role._priority.high : ''">
						<template #label>{{ i18n.ts._role.priority }}</template>
					</MkRange>
				</div>
			</MkFolder>
