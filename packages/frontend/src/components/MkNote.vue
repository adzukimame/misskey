...
>
	...
	<article v-else :class="$style.article" @contextmenu.stop="onContextmenu">
		...
		<MkAvatar :class="$style.avatar" :user="appearNote.user" :link="!mock" :preview="!mock" :instanceIcon="showInstanceIcon"/>
		<div :class="$style.main">
			...
			<MkReactionsViewer v-if="appearNote.reactionAcceptance !== 'likeOnly'" :isDetailed="false" :note="appearNote" :maxNumber="16" @mockUpdateMyReaction="emitUpdReaction">
				<template #more>
...
import { getCopyNoteLinkMenu, getNoteClipMenu, getNoteMenu, getRenoteMenu } from '@/scripts/get-note-menu.js';
...
function onContextmenu(ev: MouseEvent): void {
	...
	if (defaultStore.state.useReactionPickerForContextMenu) {
		...
		const { menu, cleanup } = getNoteMenu({ note: note.value, isDeleted, currentClip: currentClip?.value });
